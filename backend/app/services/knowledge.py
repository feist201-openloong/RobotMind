...
class KnowledgeService:
    def create_category(self, db: Session, *, name: str, description: Optional[str] = None,
    ...
    def get_categories(self, db: Session) -> List[KnowledgeCategory]:
        return db.query(KnowledgeCategory).all()

    def create_entry(self, db: Session, *, title: str, content: str,
    ...
    def search_entries(self, db: Session, *, keyword: Optional[str] = None,
    ...
    def get_entry(self, db: Session, entry_id: int) -> Optional[KnowledgeEntry]:
        return db.query(KnowledgeEntry).filter(KnowledgeEntry.id == entry_id).first()

    def update_entry(self, db: Session, entry: KnowledgeEntry, **fields) -> KnowledgeEntry:
